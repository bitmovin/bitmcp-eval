...
export class McpRecordingProxy {
  ...
  private async handle(
    ...
    if (this.opts.injectionHeaders) {
        for (const header of this.opts.injectionHeaders) {
        if (header.value === undefined) continue;
        const name = header.name.toLowerCase();
        // TODO check for any "bad" headers here and reject if needed
        fwdHeaders[name] = Array.isArray(header.value) ? header.value.join(", ") : header.value;
        }
    }
  ...
  private onMessage(m: any, pending: Map<string | number, Pending>): void {
